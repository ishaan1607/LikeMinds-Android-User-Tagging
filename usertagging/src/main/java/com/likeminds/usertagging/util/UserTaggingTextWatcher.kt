...
class UserTaggingTextWatcher(private val editText: EditText) : TextWatcher {
        ...
        try {
            ...
            if (backSpace) {
                ...
                if (spanToRemove != null) {
                    ...
                    textWatcherListener?.onUserTagRemoved(memberViewDataSpan.regex)
                }
            } else {
    ...
    fun hitUserTaggingApi() {
        if (globalPosition >= 0 && editText.selectionStart > globalPosition) {
            ...
            textWatcherListener?.hitTaggingApi(text)
        }
    }
    ...
    fun resetGlobalPosition() {
        handler.removeDelay()
        globalPosition = -1
        textWatcherListener?.hideSuggestionList()
    }
}